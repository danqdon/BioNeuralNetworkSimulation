// src/ExcitatorySynapse.cpp
#include "ExcitatorySynapse.h"
#include "INeuron.h"
#include <iostream>

ExcitatorySynapse::ExcitatorySynapse(std::shared_ptr<INeuron> pre, std::shared_ptr<INeuron> post,
...
void ExcitatorySynapse::deliverSpikeFromPre(const std::shared_ptr<INeuron>& pre, double eventTime)
{
    if (pre_neuron == pre && post_neuron) {
        // Inyectar corriente excitatoria en la neurona post
...
void ExcitatorySynapse::deliverSpikeFromPost(const std::shared_ptr<INeuron>& post, double eventTime)
{
...
void ExcitatorySynapse::adjustWeight(double delta_t)
{
    if (delta_t > 0) {
        // Potenciación (LTP)
